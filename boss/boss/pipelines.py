...
class BossPipeline(object):
    def __init__(self):
        self.conn_mysql=MySQLdb.Connection(host='127.0.0.1',port=3306,user='root',db='boss',charset='utf8',password='111111')
    ...
    def save(self,item):
        ...
        if result <=1000:
            ...
            # self.conn_mysql.cursor(sql,[item[''],item[''],item[''],item[''],item[''],item[''],item[''],item[''],item[''],item[''],item[''],item['']])
            sql = 'insert into t_boss (position)VALUES (%s)'
            self.conn_mysql.cursor(sql,[item['position']])
        ...
        #     hbase_table.put(row=item['']+item['']+item['']+item[''],data={'show_data':(item[''],item['']),'noshow_data':(item[''],item[''])})